...
async function main() {
    ...
    const out = execSync("genlayer deploy --contract ../contract/test_transfer3.py --rpc https://studio.genlayer.com/api").toString();
    const contract = out.match(/Contract Address': '([^']+)'/)[1];
    console.log("Contract:", contract);
    await new Promise(r => setTimeout(r, 5000));
    
    console.log("Bal before:", Number(await client.getBalance({address: account.address}))/1e18);
    ...
    await new Promise(r => setTimeout(r, 5000)); // wait for it to process
